    ...
    for cmpnd in compounds:
        ...
        for i, letter in enumerate(cmpnd):
            ...
            if len(cmpnd) == 1:
                ...
                matrix, unique_elements = addMatrix(
                    matrix, cmpnd, compounds, cur_elmt[0], unique_elements, side
                )
                break
            ...
            # Two letter element
                ...
                if i + 1 != len(cmpnd):
                    ...
                    if cmpnd[i + 1].isnumeric():
                        ...
                        # Using given number.
                        cur_elmt.append(
                            "".join([prev_letter, letter, str(coef * int(num))])
                        )
                        elements.append(cur_elmt)
                        matrix, unique_elements = addMatrix(
                            matrix, cmpnd, compounds, cur_elmt[0], unique_elements, side
            ...
            # Single letter element
                ...
                if i + 1 != len(cmpnd) and (
                    not (cmpnd[i + 1].islower() and cmpnd[i + 1].isalpha())
                ):
